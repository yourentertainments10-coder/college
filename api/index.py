...
def init_db():
    ...
    try:
        ...
        cursor.execute("""
            ...
            CREATE TYPE IF NOT EXISTS user_type AS ENUM ('faculty', 'admin');
            CREATE TYPE IF NOT EXISTS enrollment_status AS ENUM ('active', 'dropped', 'completed');
            CREATE TYPE IF NOT EXISTS attendance_status AS ENUM ('present', 'absent', 'late');
            CREATE TYPE IF NOT EXISTS grade_type AS ENUM ('assignment', 'midterm', 'final', 'project');
            CREATE TYPE IF NOT EXISTS notification_type AS ENUM ('info', 'warning', 'success', 'error');
            CREATE TYPE IF NOT EXISTS schedule_type AS ENUM ('lecture', 'lab', 'office_hours');
            CREATE TYPE IF NOT EXISTS day_of_week AS ENUM ('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday');
        """)
...
# Do not call init_db() on every startup for serverless; run migrations separately
# init_db()  # Comment out for production
